...
async def show_city(ctx: commands.Context, *, city_name=""):
    if not city_name:
        ...
        return
    manager.create_graph(f'{ctx.author.id}.png', [city_name])  # Membuat peta untuk kota yang diberikan
    await ctx.send(file=discord.File(f'{ctx.author.id}.png'))
@bot.command()
async def show_my_cities(ctx: commands.Context):
    cities = manager.select_cities(ctx.author.id)  # Mengambil daftar kota yang disimpan oleh pengguna
    if cities:
        manager.create_graph(f'{ctx.author.id}_cities.png', cities)  # Membuat peta dengan semua kota yang disimpan oleh pengguna
